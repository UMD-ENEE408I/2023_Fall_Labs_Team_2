...
const unsigned int MAX_PWM_VALUE = 150; // Max PWM given 8 bit resolution
...
void set_motors_pwm(float left_pwm, float right_pwm) {
  ...
  if (left_pwm  >  150.0) left_pwm  =  150.0;
  if (left_pwm  < -150.0) left_pwm  = -150.0;
  if (isnan(right_pwm)) right_pwm = 0.0;
  if (right_pwm >  150.0) right_pwm =  150.0;
  if (right_pwm < -150.0) right_pwm = -150.0;
...
void cardoid(float t, float a, float& x, float& y) {
  x = a*cos(t)*(0.5-cos(t));
  y = a*sin(t)*(0.5-cos(t));
}
...
void loop() {
  ...
  float leminscate_t_scale = 1.0; // speedup factor
  float x0, y0;
  cardoid(0.0, leminscate_a, x0, y0);
  float last_x, last_y;
  cardoid(-leminscate_t_scale * target_period_ms / 1000.0, leminscate_a, last_x, last_y);
  ...
  while (true) {
    ...
    // of 0.5 meter radius
    float x, y;
    cardoid(leminscate_t_scale * t, leminscate_a, x, y);
